...
// Enforce Role
if (!isset($_SESSION['2fa_role']) || !in_array($_SESSION['2fa_role'], ['inventory', 'admin', 'super_admin'])) {
    header('Location: login.php');
    exit();
}
...
<head>
    ...
    <title>Inventory Dashboard - Le Maison</title>
...
<body>
    ...
    <section id="inventory-section" class="section-view">
        <div class="header">
            ...
            <div style="display:flex; gap:10px;">
                <button class="btn-action" onclick="window.printInventorySheet()" style="background:var(--dark-brown);">
                    <i class="fas fa-print"></i> Print Sheet
                </button>
                <button class="btn-action" onclick="window.openInventoryModal()" id="invAddItemBtn">
                    <i class="fas fa-plus"></i> Add Item
                </button>
            </div>
        </div>
            ...
            <form id="inventoryForm">
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div class="form-group">
                        ...
                        <input type="number" id="invQuantity" class="form-control" min="0" step="any" required>
                    </div>
                    <div class="form-group">
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div class="form-group">
                        ...
                        <input type="text" id="invSupplier" class="form-control" placeholder="Name / Contact">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Expiry Date (Optional)</label>
                        <input type="date" id="invExpiry" class="form-control">
                    </div>
                </div>
                <div class="form-group">
    ...
    <div id="manageStockModal" class="modal">
        <div class="modal-content" style="max-width: 400px;">
            <div class="modal-header">
                ...
                <button class="btn-icon" onclick="closeModal('manageStockModal')"><i class="fas fa-times"></i></button>
            </div>
            <form id="manageStockForm">
                <input type="hidden" id="manageStockId">
                <div style="margin-bottom: 15px; text-align: center;">
                    <h4 id="manageStockTitle" style="color:var(--primary-gold); margin: 0;"></h4>
                    <p style="font-size: 0.85rem; color:var(--text-muted); margin: 4px 0 0 0;">Current: <strong id="manageStockCurrentDisplay"></strong></p>
                </div>

                <div class="form-group">
                    <label class="form-label">Action</label>
                    <select id="manageStockAction" class="form-control" required onchange="window.toggleSpoilageReason()">
                        <option value="Stock In">Stock In (Add)</option>
                        <option value="Stock Out">Stock Out (Deduct)</option>
                        <option value="Spoilage">Log Spoilage / Wastage</option>
                    </select>
                </div>
                <div class="form-group">
                    <label class="form-label">Quantity to Adjust</label>
                    <input type="number" id="manageStockQty" class="form-control" min="0.01" step="any" required>
                </div>
                <div class="form-group" id="spoilageReasonGroup" style="display:none;">
                    <label class="form-label">Reason / Remarks</label>
                    <input type="text" id="manageStockRemarks" class="form-control" placeholder="e.g. Expired, spilled, kitchen use">
                </div>
                <button type="submit" class="btn-submit" id="saveManageStockBtn">Confirm Action</button>
            </form>
        </div>
    </div>
